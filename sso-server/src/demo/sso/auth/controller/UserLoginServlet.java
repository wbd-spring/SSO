		...
		try {
			...
			if (user != null) {
				if (user.getPasswd().equals(passwd)) { // 判断密码是否正确
					...
					tokenCookie.setHttpOnly(true); //为了token安全化， 不让js去操作，后面通过jsonp进行操作。
					response.addCookie(tokenCookie);  //把token写出到客户端。 key=token value=token的值，也是uuid生成的值

					// 4 跳转到原请求
					if (StringUtil.isEmpty(origUrl)) {
						origUrl = "login_success";
					} else {
						System.out.println("登錄成功....."+origUrl);
